...
def calc_s_test_single(b_input_ids, b_attn_mask, b_labels, b_claims, model, train_loader, gpu=-1,
    ...
    s_test_vec_list = []
    for i in range(r):
        s_test_vec_list.append(s_test(b_input_ids, b_attn_mask, b_labels, b_claims, model, train_loader,
...
def calc_influence_single(model, train_loader, test_loader, test_id_num, gpu,
    ...
    for i in range(train_dataset_size):
        ...
        grad_z_vec = grad_z(b_input_ids, b_attn_mask, b_labels, b_claims, model, gpu=gpu)
        if time_logging:
